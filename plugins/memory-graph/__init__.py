...
def _get_admin_ids() -> set:
    ...
    try:
        ...
        pass
    # Fallback: 左灏(Nitrogen) is the server owner
    return {"telegram:7359770766"}
...
def _pre_llm_call(user_message="", **kwargs):
    ...
    try:
        ...
        # Format results as brief context within a configurable budget.
        parts = []
        total_len = 0
        budget = int(cfg.get("auto_recall_context_chars", _context_budget()))
        for r in results[:max_results]:
            uri = r.get("uri", "")
            snippet = r.get("snippet", "")[:220]
            line = f"[Memory Graph] {uri}: {snippet}"
...
def _core_item_safe_for_shared_chat(item: dict) -> bool:
    ...
    if path.startswith(blocked_path_prefixes) or "用户档案/" in path:
        return False
    private_markers = (
        "Steven", "左灏", "Nitrogen", "7359770766", "7910206541",
        "家庭", "爸爸", "妈妈", "妹妹", "经济", "生日", "地址", "电话",
        "password", "token", "api key", "ghp_", "sk-",
    )
    hay = path + "\n" + content
