  ...
  return (
    <div className="mt-2 border border-warning/20 bg-warning/5">
      <ListItem
        ...
        aria-expanded={open}
        className="min-w-0 px-3 py-2 text-xs text-warning hover:bg-warning/10 hover:text-warning"
      >
        {open ? (
        ...
        <span className="min-w-0 truncate font-mono-ui font-medium">
          {toolCall.function.name}
        </span>
        <span className="ml-auto max-w-[40%] truncate text-warning/50">{toolCall.id}</span>
      </ListItem>
      {open && (
        <pre className="max-w-full overflow-x-auto whitespace-pre-wrap break-words border-t border-warning/20 px-3 py-2 font-mono text-xs text-warning/80">
          {args}
        </pre>
      )}
    </div>
...
function MessageBubble({
  ...
  return (
    <div
      className={`min-w-0 max-w-full overflow-hidden ${style.bg} p-3 ${isHit ? "ring-1 ring-warning/40" : ""}`}
      data-search-hit={isHit || undefined}
    >
      <div className="mb-1 flex min-w-0 flex-wrap items-center gap-2">
        <span className={`min-w-0 max-w-full truncate text-xs font-semibold ${style.text}`}>{label}</span>
        {isHit && (
        ...
        <>
          ...
          {!searchResults && total > PAGE_SIZE && (
            <div className="flex min-w-0 flex-col gap-2 pt-2 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex min-w-0 items-center gap-1 self-end sm:self-auto">
                <Button
                  outlined
                  size="icon"
                  disabled={page === 0}
