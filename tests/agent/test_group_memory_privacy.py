...
def _make_agent(chat_type="group"):
    agent = AIAgent(
        ...
        chat_id="-100group" if chat_type != "dm" else "sender-private-user",
        chat_name="港大預備役學生交流群" if chat_type != "dm" else None,
        chat_type=chat_type,
        session_id="sess-group" if chat_type != "dm" else "sess-dm",
