...
async def get_sessions(limit: int = 20, offset: int = 0, source: str = ""):
    try:
        ...
        try:
            ...
            sources = [dict(row) for row in source_cur.fetchall()]
            return {
                "sessions": sessions,
                "total": total,
                "limit": limit,
...
def _build_models_bootstrap() -> dict:
    """Best-effort Models page bootstrap data injected into index.html.

    This keeps the Models page usable on iPad/Safari even if subsequent
    browser fetches fail with WebKit's generic "Load failed".
    """
    data: dict = {}
    try:
    ...
    try:
        ...
        data["show_token_analytics"] = bool((cfg.get("dashboard") or {}).get("show_token_analytics") is True) if isinstance(cfg.get("dashboard"), dict) else False
    except Exception:
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        bootstrap_json = json.dumps(_build_models_bootstrap(), ensure_ascii=False).replace("</", "<\\/")
        token_script = (
