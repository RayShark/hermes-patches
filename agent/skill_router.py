...
def _parse_manifest_from_skill_info(cmd: str, info: Mapping[str, Any]) -> SkillManifest:
    ...
    tags = _as_list(fm.get("tags")) + _as_list(hermes.get("tags"))
    triggers = _as_list(fm.get("triggers")) + _as_list(hermes.get("triggers")) + _as_list(hermes.get("triggered_by"))
    required_when = _as_list(fm.get("required_when")) + _as_list(hermes.get("required_when"))
    return SkillManifest(
        ...
        tags=tags,
        triggers=triggers,
        negative_triggers=_as_list(fm.get("negative_triggers")) + _as_list(hermes.get("negative_triggers")),
        task_types=_as_list(fm.get("task_types")) + _as_list(hermes.get("task_types")),
        required_when=required_when,
        dependencies=_as_list(fm.get("dependencies")) + _as_list(hermes.get("dependencies")),
        conflicts=_as_list(fm.get("conflicts")) + _as_list(hermes.get("conflicts")),
        priority=int(fm.get("priority") or hermes.get("priority") or 0),
        load_policy=str(fm.get("load_policy") or hermes.get("load_policy") or "optional"),
        examples_positive=_as_list(fm.get("examples_positive")) + _as_list(hermes.get("examples_positive")),
        examples_negative=_as_list(fm.get("examples_negative")) + _as_list(hermes.get("examples_negative")),
        verification_hooks=_as_list(fm.get("verification_hooks")) + _as_list(hermes.get("verification_hooks")),
        command_key=cmd,
...
def build_autoload_skill_messages(user_message: str, *, task_id: str | None = None) -> tuple[list[dict[str, str]], SkillRouteDecision]:
    commands = _skill_command_map()
    names = available_skill_names_from_commands(commands)
    manifests = load_skill_manifests(commands)
    decision = route_skills(user_message, names, manifests=manifests)
